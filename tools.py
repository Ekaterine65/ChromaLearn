...
def keyword_to_emoji(word: str | None) -> str:
    return KEYWORD_EMOJI.get((word or "").lower().strip(), FALLBACK_EMOJI)
...
def task_to_game_dict(task: Task) -> dict:
    ...
    return {
        ...
        "title": task.title,
        "emoji": keyword_to_emoji(emotion.name if emotion else None),
